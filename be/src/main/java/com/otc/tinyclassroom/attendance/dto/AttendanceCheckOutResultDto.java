 ...
 * @param status
 */
public record AttendanceCheckOutResultDto(Timestamp checkOutTime, int status) {
    // 생성자 및 필요한 메소드들은 record에서 자동 생성됩니다.

    // from 메소드 추가
    public static AttendanceCheckOutResultDto from(Timestamp checkOutTime, int status) {
